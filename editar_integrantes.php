...
<html lang="es">
      ...
      <body>
        ...
        <h2 id="titulo_gestion"> Editar una entrada de integrante: </h2>
        <form id="formulario_gestion" name="formulario" method="POST" action="editar_integrante.php">
                    ...
                    <tr>
                        <td>
                            <label> Título: <br/>
                                ...
                                echo '<select id="tp" name="integrante">';
                                while ($registroActual = mysqli_fetch_array($resultado)) {
                                    echo '<option name="integrante" value="'.$registroActual['nombre'].'">'.$registroActual['nombre'].'</option>';
                                }
                                echo '</select>';
                                ?>
                            </label>
        ...
        if(isset($_GET['error'])){
            ...
            if(array_key_exists('nombre_grupo', $error)){
                echo $error['nombre_grupo'];
            }
            if(array_key_exists('descripcion_grupo', $error)){
                echo $error['descripcion_grupo'];
            }
            if(array_key_exists('foto_grupo', $error)){
                echo $error['foto_grupo'];
            }
        }
        ?>
        <h2 id="titulo_gestion">Añadir un nuevo miembro al grupo</h2>
        <form id="formulario_gestion" method="post" action="comprobar_integrante.php">
            <fieldset id="registrarse">
                <table class="tabla_gestion" cellpadding="10">
                    <tr>
                        <td>
                            <label> Titulo
                              <textarea name="nombre_grupo" class="caja_texto" type="text"> </textarea>
                            </label>
                        </td>
                        <td>
                            <label> Parrafo
                              <textarea name="descripcion_grupo" class="caja_texto" type="text"> </textarea>
                            </label>
                        </td>
                        <td>
                            <label> Foto (link)
                              <textarea name="foto_grupo" class="caja_texto" type="text"> </textarea>
                            </label>
                        </td>
                    </tr>
                </table>
        ...
        <h2 id="titulo_gestion"> Eliminar un integrante del grupo: </h2>
        <form id="formulario_gestion" name="formulario" method="POST" action="elimina_integrante.php" enctype="multipart/form-data">
            <fieldset class="iniciarSesion">
                <table class="tabla_gestion">
                    <tr>
                        <td>
                            <label>
                                ...
                                $consulta = "SELECT nombre FROM grupo";
                                $resultado = consultarA($consulta);
                                echo '<select id="tp" name="integrante_borrar">';
                                while ($registroActual = mysqli_fetch_array($resultado)) {
                                    echo '<option name="integrante_borrar" value="'.$registroActual['nombre'].'">'.$registroActual['nombre'].'</option>';
                                }
                                echo '</select>';
                                ?>
                            </label>

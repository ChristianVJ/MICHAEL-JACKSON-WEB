...
<html lang="es">
      ...
      <head>
          <title> Michael Jackson - Borrar tabla </title>
      ...
      <body>
        ...
        <form id="formulario_gestion" name="formulario" method="POST" action="elimina_tabla.php" enctype="multipart/form-data">
          <h2 id="titulo_gestion"> Borrar tabla: </h2>
                    ...
                    <tr>
                        <td>
                            <label>
                                Tabla: <br/>
                                <?php
                                $consulta = "SELECT TABLE_NAME as name FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'christianam1718'";
                                $resultado = consultarA($consulta);
                                echo '<select id="tp" name="tabla_borrar">';
                                while ($registroActual = mysqli_fetch_array($resultado)) {
                                    echo '<option name="tabla_borrar" value="'.$registroActual['name'].'">'.$registroActual['name'].'</option>';
                                }
                                echo '</select>';
                                ?>
                            </label>
